...
private val maxTime = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
    Instant.now()
} else {
    TODO("VERSION.SDK_INT < O")
}
private val minTime = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
    maxTime.minus(2, ChronoUnit.DAYS)
} else {
    TODO("VERSION.SDK_INT < O")
}
